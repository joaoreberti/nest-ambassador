...
export class AuthController {
  ...
  async login(
    ...
    const resp = await axios.post('http://172.17.0.1:8001/api/login', {
      email,
      password,
      scope:
        request.path === '/api/admin/login' ? 'admin' : 'ambassador',
    });
  ...
  async user(@Req() request: Request) {
    ...
    const resp = await axios.get('http://172.17.0.1:8001/api/user', {
      headers: {
        Cookie: `jwt=${cookie}`,
      },
    });
